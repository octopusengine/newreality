...
#import oe_incl
import oeLab #for raspberry pi


nexThread = True #running
timer1=0.2        # every timer1 sec...
...
def plotChartLab(x,y,dataG3,dataMax): # test for procesor temperature
 if (pGraf.enable):
  i=0
  gWidth=300
  iNext = gWidth/150 #a2
  ...
  delL=2
  for num3 in dataG3:
    yP=y-float(num3[0]/delL)
    pygame.draw.line(window,cRED,(x+i,yP),(x+i,yP+2),2)
    yP=y-float(num3[1]/delL)
    pygame.draw.line(window,cBLU,(x+i,yP),(x+i,yP+2),2)   
    yP=y-float(num3[2]/delL)
    pygame.draw.line(window,cWHI,(x+i,yP),(x+i,yP+2),2)   
...
def doPluginsBefore():
    ...
    plotChartLab(pGraf.x,pGraf.y,labData,256)
    line3dlist(xyzList)
...
def nexth(): ##thread timer
 ...
 while nexThread:
   ...
   labD=oeLab.getLabData()
   #print labData[0]
   labData.append((labD[0],labD[2],labD[4]))
